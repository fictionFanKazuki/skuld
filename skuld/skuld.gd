...
var can_blink = true # set to false when in blink to prevent conflicts
var hand_projectile

# array of active callback promises [[f, a, start_tick, duration],]
var promises: Array

func spawn_small_clock_hand(proj_x, proj_y, dir):
	# probably the most important bit, gives us a way of accessing the
	# projectile later
	# spawn object i mean idk you just feed it values and it spits out an
	# object identifier thing
	var obj = spawn_object(
		preload("res://characters/skuld/projectiles/small_clock_hand/SmallClockHand.tscn"),
		proj_x,
		proj_y,
		false,
		{"dir": dir}
